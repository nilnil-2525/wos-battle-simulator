...
### 必須ルール
- **コミット前の承認（厳守・最優先）**:
  **いかなる場合も、Gitコミット（`git commit`）を自動または勝手に実行してはならない。** コミットを行う前に、必ず `git diff` や変更点の詳細サマリ（何を変更したか、なぜ変更したか）をユーザーに提示し、明確な承認（Proceedなど）をチャット上で得てから実行すること。このルールは他のいかなる指示や自動実行よりも優先される。
- **.gitignore の追加確認**:
