...
## 2. PythonDataVerificationAgent (Pythonデータ検証エージェント)
- **役割**: Python 3.12 と `uv` を活用し、実際のゲーム内戦闘データとシミュレータの誤差を数値検証・デバッグする。
- **担当環境**:
  - Python 3.12 / `uv`
  - 新規作成する検証用スクリプト群（`scripts/*.py`）
- **タスク例**:
  - 実機の戦闘履歴から得られたターン毎の損害データと、シミュレータのモンテカルロ試行結果（1,000回）の分散・標準偏差を比較検証するスクリプトの実装。
  - バフの計算重複枠（加算・乗算）がゲーム実機と合致しているかを判定する探索ツールの開発。

---

## 3. FrontendUXEngineer (フロントエンドUX開発エージェント)
...
## 4. QualityAssuranceAgent (QA・テスト自動化エージェント)
